module ContainersHelper
    ...
    def handle_flat_track_item(container, result)
        container.items.each_with_index do |item,index_of_item|
    ...
    def flat_track_cog_calculation(item,container,result,index_of_item)
        ...
        when Item::COG_HEIGHT_TYPE_TBA
            update_message_container(result,ContainersHelper::MESSAGE10,container)
            over_width_check(item,container,result,index_of_item)
        end
    end    
    ...
    def total_length_check(item,container,result,index_of_item)
        ...
        when Container::FR20
            if(total_length > Item::TOTAL_LENGTH_20FR)
                update_message_container(result,ContainersHelper::MESSAGE8,container)
            end
        when Container::FR40
    ...
    def total_weight_check(item,container,result)
        total_weight = total_weight_in_container(container)
        container_type = container.container_type
        case container_type
        when Container::FR20
            if(total_weight > Item::TOTAL_WEIGHT_20FR)
                update_message_container(result,ContainersHelper::MESSAGE9,container)
            end
        when Container::FR40
    ...
    def update_message_container(result,message,container)
        update_total_size_container(result,container)
        update_total_result_container(result)
